...
function ProblemDisplayTable(props) {
	const {problems, problemType} = props;
	const url = problemType === '객관식문제' ? 'multiplechoice' : 'shortans';
	return (
			...
			<tbody>
				{
					problems.map((item, index) => {
						return (
							<tr key = {index} onClick={() => props.history.push(`/multiplechoice/view?id=${item.id}`)}>
								<th>{item.id}</th>
								<td>{item.name}</td>
								<th>{item.level}</th>
