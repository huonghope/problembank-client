...
import ShortProblemDisplayTable from '../../../components/ShortProblemDisplayTable';
...
function Short(props) {
	...
	const [problemType, setProblemType] = useState('단답형 문제');
	...
	const handleDelete = async (id) => {
		if (window.confirm('해당하는 문제 삭제하시겠습니까?')) {
			...
			const res = await AdminProblemAPI.deleteShortProblem(params);
			if (res.result) {
				alert('삭제되었습니다');
	...
	const handleView = async (id) => {
		props.history.push(`/shortans/view?id=`+id);
	};

	const blockFotter = title && resultProblems.length === 0 ? {display: 'none'} : {display: 'block'};
	return (
		<DashboardLayout>
			<p style={{marginBottom: '20px'}}><i className="fa fa-search"></i> 단답형 문제 조회</p>
